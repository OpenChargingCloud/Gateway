    ...
    public partial class Gateway
    {
        ...
        public JObject DNSConfigurationJSON()
            ...
            => new (
                   ...
                   // The servers this gateway would ask, which is not the same
        ...
        /// <summary>
        /// Change how this gateway resolves names, at once and for everything
                                                 ...
                                                 [NotNullWhen(false)] out String?  Error)
        {
            ...
            {
                Error = "A gateway that resolves no names cannot reach anything. Switch name resolution off instead of emptying the list.";
                return false;
            }

            // Under the same lock as every other change to this gateway:
        ...
        private void ApplyDNSConfiguration(DNSConfiguration Configuration)
        {
            ...
            // must end up holding exactly the servers this gateway means it to
        ...
        /// <summary>
        /// Where this gateway gets the time from, and how the key exchange
        /// behind it is doing.
        /// </summary>
        /// <remarks>
        ...
        /// "keyExchange" - which read as the gateway's time server and the
        ...
        public JObject NTSConfigurationJSON()
        {
            ...
            return new JObject(
                       ...
                       // it is in effect. The quorum is the one this gateway
                       ...
                       // synchronises this gateway's clock: each server's key
        ...
        /// <summary>
        /// Change where this gateway reads the time.
        /// </summary>
        /// <remarks>
        /// Pointing the gateway at another server replaces the client rather
                                                 ...
                                                 [NotNullWhen(false)] out String?  Error)
        {
            ...
            try
            {
                ...
                // section the next start refuses, and a gateway that does not
        ...
        public static JObject? RootCAJSON(NTSKE_TLSInfo? TLS)
        {
            ...
            return root is null
                       ? null
                       : new JObject(
                             new JProperty("name",         Certificates.CommonNameOf(root)),
                             new JProperty("subject",      root.Subject),
                             new JProperty("fingerprint",  Certificates.ThumbprintOf(root))
                         );
        ...
        /// servers in effect, which the section cannot know and this gateway
        /// does.
        /// </remarks>
                                          ...
                                          [NotNullWhen(false)] out String?  Error)
        {
                ...
                Configuration.Hostname   is null)
            {
                ...
                if (quorum > asked)
                {
                    Error = $"'nts.minServers' is {quorum}, which is more servers than the {asked} this gateway asks.";
                    return false;
                }
        ...
        private void ApplyNTSConfiguration(NTSConfiguration Configuration)
        {
            ...
            // much later. See Gateway.Clock.cs.
            //
                ...
                ntp      != ntsClient.NTP_Port)
            {
                ...
                // about the last exchange belongs to a server this gateway no
                // longer asks.
                lastTimeSync = null;
            ...
            // The clock is checked on a timer set when the gateway started, so
