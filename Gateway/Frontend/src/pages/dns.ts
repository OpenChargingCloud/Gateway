 ...
 * inside the gateway that resolves anything - the name servers included, which
...
export const dnsPage: Page = {
    ...
    render({ root }) {
        ...
        const content = shell(root, {
            ...
            title:     'DNS client',
            subtitle:  'How this gateway resolves names.',
            ...
            render(content, html`
                ...
                <div class="cards stacked">
                    ...
                    <section class="card">
                        ...
                        <p class="hint">
                            Switched off, this gateway asks nobody: the name servers are taken away from
                    ...
                    <section class="card">
                        ...
                        <p class="hint">
                            ${mayTest
                                  ? html`
                                        This asks the way the gateway asks for anything: the servers above, in
                    ...
                    <section class="card">
                        ...
                        <p class="hint">Changing these means making another client, which means restarting the gateway.</p>
                    </section>
        ...
        async function save(update: DNSUpdate): Promise<void> {
            ...
            try
            {
                ...
                // the page shows what the gateway took rather than what the
                // form sent.
         ...
         *
         * The gateway tries them in turn, so this is the list the page has to
         * be willing to wait for - what it is given up on is the gateway going
         * quiet, never the patience the gateway was configured with.
         */
         ...
         * after twenty, because the gateway tries again. A deadline of ten
         * would have given up on a gateway that was still doing what it was
         * told.
         */
         ...
         *                null to ask the way the gateway asks for anything.
         */
            ...
            function paint(): void {
                ...
                render(dialog, html`
                    ...
                    <p class="hint">
                        ${asking === null
                              ? html`
                                    Asked the way this gateway asks for anything: the configured servers, in
        ...
        // looks untouched - so it is compared with what the gateway last said.
