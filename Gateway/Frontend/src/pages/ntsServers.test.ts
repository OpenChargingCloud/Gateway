/**
 * What the NTS page tells the gateway when a time server is added, edited or
 * deleted, asked directly.
 *
 * Run with `npm test`. The gateway is sent the whole list every time, so what
...
/** A server as the gateway shows it. */
...
describe('the list a change sends', () => {
    ...
    it('leaves the list it was made from alone, which is what the page goes back to when the gateway says no', () => {
