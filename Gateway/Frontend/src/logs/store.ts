...
// The browser's copy of the gateway's log, fed by two things: a snapshot from
// the JSON API and the Server-Sent Events stream. Both carry ids from the same
// counter on the gateway, and the counter only ever grows - so an entry is
...
/**
 * How many entries the page keeps. The gateway keeps its own number and
...
/**
 * How long after the stream has given up before the gateway is asked why.
 *
 * Measured: the gateway was stopped and started again, which takes every
 ...
 * frozen list for as long as it was left open, with the gateway up and running
 * and the operator signed out without being told.
 *
 * So when the browser gives up, the gateway is asked why. A 401 is the answer,
...
/**
 * And how long before asking again, where the gateway could not answer either.
 *
 * A pace that suits a page somebody has left open in front of a gateway that
...
export class LogStore {
    ...
    /** Every tag the gateway has seen, for the filter. */
    ...
    /** How many entries the gateway keeps. */
    capacity = 0;

    private source:              EventSource | null = null;
    private readonly listeners = new Set<Listener>();

    /** Set while the gateway is being asked why the stream stopped. */
    ...
    /**
     * Why the stream stopped, asked of the gateway rather than guessed.
     *
        ...
        this.askAgain = setTimeout(() => {
            ...
            api.auth.me().then(
                ...
                (problem: unknown) => {
                    ...
                    // The gateway could not answer either, so it is still
    ...
    /** Throw away what this page shows, without touching the gateway's log. */
    clear(): void {
        this.entries.length = 0;
    ...
    /** The snapshot: what happened up to now, as far back as the gateway keeps. */
