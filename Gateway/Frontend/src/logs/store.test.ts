 ...
 *
 * What is pinned is what was measured to be wrong: the gateway was stopped and
 ...
 * "reconnecting ..." over a frozen list, with the gateway up and running and
...
/** What the gateway was asked, and what it answered. */
let askedFor: string[] = [];

const gatewayAnswers = (How: 'yes' | 'signed out' | 'not there') => {
...
describe('a stream that stops', () => {
    ...
    it('is left to the browser while the browser is still trying', async () => {
        ...
        try
        {
            ...
            assert.deepEqual(askedFor, [], 'the gateway was asked about a stream nobody had given up on');
    ...
    it('makes the gateway be asked why, once the browser has given up', async () => {
        ...
        try
        {
            ...
            assert.deepEqual(askedFor, [], 'the gateway was asked before anything had settled');

            mock.timers.tick(5_000);
            await Promise.resolve();
            await Promise.resolve();

            assert.equal(askedFor.length, 1, 'nobody asked the gateway anything');
    ...
    it('is opened again where the gateway is there and the session still stands', async () => {
    ...
    it('is not opened again when the answer is that nobody is signed in', async () => {
        ...
        try
        {
            ...
            // same refusal.
            assert.equal(Stream.latest, stream, 'a new stream was opened at a gateway that had refused');
            ...
            assert.equal(askedFor.length, askedOnce,
                         'it went on asking a gateway that had already said no');
    ...
    it('keeps trying where the gateway could not answer either', async () => {

        mock.timers.enable({ apis: ['setTimeout'] });

        try
        {
            gatewayAnswers('not there');
            ...
            // A gateway that is being restarted is away for longer than one
