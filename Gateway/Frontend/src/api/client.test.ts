/**
 * What the web interface does when the gateway does not answer.
 *
 ...
 * requests left hanging by seven clicks, and a gateway that could not be
 * reached described to the operator in the browser's words rather than in
 * words about a gateway.
 */
...
/** A gateway that takes the request and then says nothing, ever. */
...
describe('a gateway that does not answer', () => {
    ...
    it('is given up on, rather than waited for as long as the browser is willing to', async () => {
        ...
        assert.ok(problem instanceof NoAnswer,
                  `a silent gateway produced ${problem} rather than a NoAnswer`);
    ...
    it('is given up on even when it is silent halfway through the body', async () => {
        ...
        // A gateway that sends its headers and then stops hangs exactly as
    ...
    it('is described in words about a gateway, not the browser\'s', async () => {

        // What the browser says here is "Failed to fetch", which names neither
        // the gateway nor anything to do about it.
        ...
        assert.match((problem as Error).message, /gateway/);
...
describe('what somebody is told when it runs out', () => {
    ...
    it('does not claim a write was not carried out, because nobody knows that', async () => {
        ...
        // The page stopped waiting; the gateway may well have written the file
...
describe('a gateway that does answer', () => {
    ...
    it('and says no is still an ApiError, with the gateway\'s own sentence', async () => {
...
describe('how long the page is willing to wait', () => {
    ...
    it('allows far more than the gateway has ever needed', () => {

        // Every read and write of this gateway's own configuration measured
...
describe('signing in', () => {
    ...
    // The sign-in is the one call that does not go to this gateway's own API.
    ...
    it('answers with what the account may do, not with what the sign-in said', async () => {
        ...
        assert.deepEqual(me.roles, ['operator']);
