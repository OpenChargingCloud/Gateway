...
namespace cloud.charging.open.Gateway.Tests
{
    ...
    public class FileLogTests
    {
        ...
        public void Setup()
        {
            ...
            // what it promises.
            directory  = Path.Combine(Path.GetTempPath(), "gateway-file-log-" + Guid.NewGuid().ToString("N")[..12]);
        ...
        public void EverythingIsWrittenDownToTheDebugEntries()
        {
            ...
            Assert.That(File.ReadAllLines(Path.Combine(directory, "gateway-2026-09-23.log")),
                        Is.EqualTo(new[] {
        ...
        /// situation that matters: a gateway that is killed has its last lines
        /// on disk rather than in a buffer, and somebody following the file
        /// while the gateway runs sees each entry as it happens.
        /// </remarks>
        [Test]
        ...
        public void AFileIsADayAndTheDayIsUTC()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(File.ReadAllLines(Path.Combine(directory, "gateway-2026-09-24.log")),
                            Is.EqualTo(new[] {
        ...
        /// <summary>
        /// A gateway started twice on one day adds to that day's file.
        /// </summary>
        [Test]
        public void NothingThatWasThereIsOverwritten()
        {
            ...
            File.WriteAllText(Path.Combine(directory, "gateway-2026-09-23.log"), "What the morning's run wrote.\n");
        ...
        public void AFileThatCannotBeWrittenIsSaidOnceAndItsGapWrittenDown()
        {
            ...
            var blocked  = Path.Combine(directory, "gateway-2026-09-23.log");
        ...
        #region AGatewayWritesItsVeryFirstEntryIntoTheFile()

        /// <summary>
        /// The file is attached before the gateway says anything at all.
        /// </summary>
        /// <remarks>
        /// The first entry a gateway writes is that it is starting up, and a
        ...
        /// </remarks>
        [Test]
        public async Task AGatewayWritesItsVeryFirstEntryIntoTheFile()
        {
            ...
            try
            {
                ...
                var file = Directory.GetFiles(logs, "gateway-*.log").Single();
            ...
            Assert.That(lines.FirstOrDefault(), Does.Contain("[gateway] Gateway v").And.Contain("starting up."));
