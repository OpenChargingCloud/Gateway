...
namespace cloud.charging.open.Gateway.Tests
{
    ...
    public class TimeServerCertificateTests
    {
        ...
        private static (String Level, String Text)[] Steps(X509Certificate2                    Server,
            ...
            => [.. Gateway.TLSSteps(new NTSKE_TLSInfo(
        ...
        public void EveryCertificateIsShownWithBothEndsOfItsValidity()
        {
            ...
            Assert.That(steps.Select(step => step.Text), Is.EqualTo(new[] {
                ...
                $"The root's SHA-256 fingerprint: {Certificates.ThumbprintOf(root)}.",
        ...
        public void TheRootCAIsNamedAndFingerprintedForTheList()
        {
            ...
            var json = Gateway.RootCAJSON(new NTSKE_TLSInfo(ServerCertificate: server, ValidatedChain: [ server, root ]));

            Assert.Multiple(() => {
                Assert.That(json?.Value<String>("name"),         Is.EqualTo("Test Root"));
                Assert.That(json?.Value<String>("subject"),      Is.EqualTo("CN=Test Root"));
                Assert.That(json?.Value<String>("fingerprint"),  Is.EqualTo(Certificates.ThumbprintOf(root)));
                Assert.That(Gateway.RootCAJSON(null),                 Is.Null);
                Assert.That(Gateway.RootCAJSON(new NTSKE_TLSInfo()),  Is.Null, "a session that kept no chain names no root");
            });
